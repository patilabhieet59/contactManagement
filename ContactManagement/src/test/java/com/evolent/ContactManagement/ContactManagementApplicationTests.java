...
class ContactManagementApplicationTests {
    ...
    public void testAddContacts() throws Exception{
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/addContact",c1)).andExpect(MockMvcResultMatchers.status().isCreated());
    ...
    public void testUpdateContacts() throws Exception{
        ...
        Mockito.when(repository.save(Mockito.any())).thenReturn(c1);
        mockMvc.perform(MockMvcRequestBuilders.put("/updateContact",c1)).andExpect(MockMvcResultMatchers.status().isOk());

    }

    @Test
    public void testDeleteContacts() throws Exception{
//        Mockito.when(repository.deleteById(Mockito.anyLong())).thenReturn();
        mockMvc.perform(MockMvcRequestBuilders.delete("/deleteContact",1L)).andExpect(MockMvcResultMatchers.status().isMovedPermanently());
